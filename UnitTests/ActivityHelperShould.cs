...
public class ActivityHelperShould
{
    ...
    public void Should_Validate_MultiColumnActivity()
    {
        ...
        Assert.True(ActivityHelper.IsValidMultiColumnActivity(posValues, 0, 3));
        Assert.False(ActivityHelper.IsValidMultiColumnActivity(posValues, 0, 1));
    }
    ...
    public void Should_Find_Largest_Matching_Block()
    {
        ...
        Assert.Equal(2, ActivityHelper.FindLargestMatchingBlock(posValues, 0));
        Assert.Equal(2, ActivityHelper.FindLargestMatchingBlock(posValues, 2));
    }
    ...
    public void Should_Identify_Single_Column_Activity()
    {
        ...
        Assert.True(ActivityHelper.IsSingleColumnActivity(posValues, 1));
        Assert.False(ActivityHelper.IsSingleColumnActivity(posValues, 0));
    }
    ...
    public void Should_Check_If_Column_All_Empty()
    {
        ...
        Assert.True(ActivityHelper.IsColumnAllEmpty(posValues, 1));

        posValues = CreatePosValues(
            new[] { "", "C", "" },
            new[] { "", "", "" }
        );
        Assert.False(ActivityHelper.IsColumnAllEmpty(posValues, 1));
    }
    ...
    public void Should_Extract_Pos_Values()
    {
        ...
        var posValues = ActivityHelper.GetPosValues(block, 3);
    ...
    public void Should_Get_Per_Ciphers()
    {
        ...
        Assert.Equal("123", ActivityHelper.GetPerCiphers(block));
    }
    ...
    {
        var block = CreateBlock("1a", "BLOK1", "123", new[] { "A", "B", "C" });
        Assert.Equal("1a", ActivityHelper.GetKlaValue(block));
        Assert.Equal("BLOK1", ActivityHelper.GetBlokValue(block));
    }
    ...
    {
        int idx = 0;
        string per = ActivityHelper.TakePerCiphers(ref idx, 3, "1213");
        Assert.Equal("12", per);
        Assert.Equal(2, idx);

        idx = 0;
        per = ActivityHelper.TakePerCiphers(ref idx, 6, "1234");
